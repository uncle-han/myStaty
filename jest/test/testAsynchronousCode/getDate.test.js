...
test('async await 测试异步请求', async () => {
    const token = await getToken()
    expect(token).toBe(112233)
})

test('async await 处理异常请求', async () => {
    expect.assertions(1)
    try {
...
test('async await 配合 .resolves 测试异步请求', async () => {
    await expect(getToken()).resolves.toBe(112233)
})

test('async await 配合 .rejects 处理异常请求', async () => {
